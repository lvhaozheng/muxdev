...
class MockProvider:
    ...
    def run_stage(self, *, stage_id: str, task: str, worktree: Path) -> MockStageOutput:
        ...
        if stage_id in {"design_brief", "design_plan"}:
            artifact = PlanArtifact(summary="Mock design plan", steps=["clarify goals", "propose design", "review", "package design"])
            content = "# Design Plan\n\n" + artifact.summary + "\n\n```json\n" + artifact.model_dump_json(indent=2) + "\n```\n"
            return MockStageOutput(f"design/{stage_id}.md", content, artifact.summary)
