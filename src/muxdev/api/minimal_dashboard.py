...
_I18N: dict[str, dict[str, object]] = {
    ...
    "en": {
        ...
        "delivery_repair": "Delivery Repair",
        "ready": "Ready",
        "partial": "Partial",
...
function statusLabel(value){const key=String(value??'');if(!isZh())return key;if(key==='awaiting_feedback')return'等待设计反馈';return EXTRA_ZH.statuses[key]||phaseLabel(key)}
...
function setText(id,v){const e=document.getElementById(id);if(e)e.textContent=v}function chip(label,tone=''){return `<span class="chip ${esc(tone)}">${esc(label||t('none'))}</span>`}function runIdChip(runId){return runId?chip(`run ${runId}`,'info'):''}function metric(label,value,tone){return `<div class="metric"><div class="label">${esc(label)}</div><strong>${esc(value)}</strong>${tone?chip(tone,tone):''}</div>`}
...
function statusBucket(task){const status=String(task.status||''),deliverableStatus=task.deliverable_status||{},completedOk=status==='completed'&&deliverableStatus.complete===true;const pending=Number(task.pending_approvals||0)+Number(task.pending_provider_actions||0)+Number(task.pending_feedback_requests||0);if(completedOk)return'done';if(pending||['awaiting_approval','awaiting_provider_action','awaiting_feedback','paused_budget'].includes(status)||Number(task.errors||0)||['blocked','failed'].includes(status))return'action';if(status==='running')return'running';if(status==='completed')return'done';if(status==='aborted')return'stopped';if(['created','queued','pending'].includes(status)||!status)return'pending';return'blocked'}
function manualItems(overview=state.overview){const rows=[],seen=new Set();for(const row of overview?.pending_provider_actions||[]){const id=String(row.action_id||'');if(!id)continue;seen.add('provider:'+id);rows.push({...row,manual_kind:row.kind==='clarification_required'?'clarification':'provider_permission'})}for(const row of overview?.pending_approvals||[]){const id=String(row.approval_id||'');if(!id)continue;seen.add('approval:'+id);const type=String(row.type||row.approval_type||'');rows.push({...row,manual_kind:type==='plan'||type==='design'?'plan_approval':'approval'})}for(const row of overview?.action_center||[]){const approvalId=String(row.approval_id||''),actionId=String(row.action_id||'');if(approvalId&&seen.has('approval:'+approvalId))continue;if(actionId&&seen.has('provider:'+actionId))continue;const kind=String(row.kind||'');rows.push({...row,manual_kind:kind.includes('missing_deliverable')?'delivery_repair':kind.includes('recovery')?'failure_gate':kind.includes('plan')?'plan_feedback':kind.includes('provider')?'provider_permission':kind.includes('approval')?'plan_approval':'failure_gate'})}return rows}
