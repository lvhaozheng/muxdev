...
function attentionRows(){return [...state.actionRows,...state.providerRows.map(row=>({...row,kind:'provider_action'})),...state.approvalRows.map(row=>({...row,kind:'approval'}))];}
...
function standardCard(row){return `<div class="standard-card ${esc(row.status||'watch')}"><strong>${esc(standardLabel(row))}</strong><span class="${statusClass(row.status)}">${esc(tStatus(row.status)||row.status||'watch')}</span><div class="standard-grid"><span>${esc(t('current_value'))}</span><b>${esc(standardValue(row.current))}</b><span>${esc(t('target_value'))}</span><b>${esc(standardValue(row.target))}</b><span>${esc(t('evidence'))}</span><b>${esc(standardValue(row.evidence))}</b><span>${esc(t('recommended_action'))}</span><b>${esc(standardAction(row))}</b></div></div>`;}
