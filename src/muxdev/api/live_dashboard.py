...
function actionRow(row){return `<div class="action-row"><div><strong>${esc(tAction(row.kind))}</strong><div class="meta">${esc(row.why||row.reason||row.task_title||row.task_id||'')}</div><div class="chips"><span>${esc(row.run_id||row.task_id||'-')}</span><span>${esc(row.project_name||'-')}</span><span>${esc(row.stage_id||'-')}</span></div></div><div class="actions">${row.endpoint?`<button class="primary" onclick="post('${esc(row.endpoint).replace('/api','')}')">${esc(t('handled_continue'))}</button>`:''}${row.secondary_endpoint?`<button onclick="loadText('${esc(row.secondary_endpoint).replace('/api','')}','events')">${esc(t('report'))}</button>`:''}${row.command?`<button onclick="copyText('${esc(row.command)}')">${esc(t('copy'))}</button>`:''}</div></div>`;}
